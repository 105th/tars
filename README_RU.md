...
* [SCSS, SASS](http://sass-lang.com/), [LESS](http://www.lesscss.ru/) или [Stylus](http://learnboost.github.io/stylus/) в качестве препроцессора для css. Также можно использовать обычный css. Доступны sourcemaps. При выборе scss в качестве препроцессора, вы можете использовать как расширение scss, так и sass + можете миксовать их использование. Из коробки доступен [PostCSS](https://github.com/postcss/postcss). Подробности [в документации](/docs/ru/css-processing.md).
...
* Можно легко добавлять новые таски и вотчеры. Есть [примеры](/docs/ru/task-workflow.md) того, как создать и использовать новый таск или вотчер внутри TARS.
...
* Несколько режимов сборки: обычная, с минифицированными файлами, с хешем в названии css- и js-файлов для выкладки на боевые сревера.
...
**В случае использования TARS-CLI, установка gulp глобально и скачивание TARS не требуется.**

После установки всех зависимостей необходимо открыть tars-config (подробное описание опций [в документации](/docs/ru/options.md)) и настроить проект под себя.
В конфиге вы можете выбрать шаблонизатор, css-препроцессор, использование уведомлений в операционной системе, имена папок для различной статики и т.д.
...
`gulp re-init` — переинициализирует проект с заданными опциями в tars-config. Предлагается использовать данную команду, если вы инициализировали проект с неверными опциями. **При переинициализации часть папок и файлов из директории markup удаляются.**

`gulp` или `gulp build` — делает сборку проекта. Тип сборки зависит от переданных ключей вместе с этой командой. Доступные ключи:
